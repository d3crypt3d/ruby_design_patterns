...
class HomeTheaterFacade
  ...
  def watch_movie(movie)
    ["Get Ready to watch a #{movie} movie...", @popper.on,
     ...
     @projector.wide_screen_mode, @amp.on, @amp.set_dvd,
     @amp.set_surround_sound, @amp.set_volume(5), @dvd.on, @dvd.play(movie)]
  end
  ...
  def listen_to_cd(title)
    ['Get Ready for an audiopile experience...', @lights.on, @amp.on,
     @amp.set_volume(5), @amp.set_cd, @amp.set_stereo_sound,
     @cd.on, @cd.play(title)]
  end

  def end_cd
    ['Shutting down CD...', @amp.off, @amp.set_cd, @cd.eject, @cd.off]
  end

  def listen_to_radio(frequency)
    ['Tuning in the airwaves...', @tuner.on, @tuner.set_frequency(frequency),
     @amp.on, @amp.set_volume(5), @amp.set_tuner]
  end
